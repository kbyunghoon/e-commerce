...
모놀리식 아키텍처에서 MSA로 전환 시 발생하는 분산 트랜잭션 문제를 파악, 이벤트 기반 Orchestrator Saga 패턴을 통한 해결 방안 설계 문서

### 1.2 현재 시스템 구성

- **아키텍처**: Spring Boot 모놀리식
- **데이터베이스**: 단일 MySQL 인스턴스
- **분산처리**: 이벤트 기반 Orchestrator Saga 패턴 적용 완료
...
## 3. 이벤트 기반 Orchestrator Saga 패턴 설계
...
#### 3.1.2 Orchestrator vs Choreography

**Orchestrator 패턴 선택 이유**

- 중앙 집중식 제어로 복잡한 비즈니스 플로우 관리 용이
- 트랜잭션 상태 추적 및 모니터링 단순화
- 보상 트랜잭션 순서 제어 가능
- 디버깅 및 장애 추적 용이
...
#### 3.2.2 도메인별 이벤트 핸들러 분리

```kotlin
// 각 도메인별 독립적인 이벤트 핸들러
@Component
class BalanceSagaEventHandler(private val balanceService: BalanceService)

@Component
class ProductSagaEventHandler(private val productService: ProductService)

@Component
class CouponSagaEventHandler(private val couponService: CouponService)

@Component
class OrderSagaEventHandler(private val orderService: OrderService)
```
...
sequenceDiagram
    ...
    participant BalanceHandler as Balance<br/>Handler
    participant ProductHandler as Product<br/>Handler
    participant CouponHandler as Coupon<br/>Handler
    participant OrderHandler as Order<br/>Handler
    participant Redis
    ...
    PaymentOrchestrator ->> PaymentOrchestrator: publishEvent(PaymentSagaStartedEvent)
    Note over PaymentOrchestrator: Step 1: Balance Deduction
    PaymentOrchestrator ->> BalanceHandler: BalanceDeductionRequestedEvent
    BalanceHandler ->> BalanceHandler: balanceService.use(command)
    BalanceHandler ->> PaymentOrchestrator: BalanceDeductedEvent
    Note over PaymentOrchestrator: Step 2: Stock Deduction
    PaymentOrchestrator ->> Redis: markStepCompleted(DEDUCT_BALANCE)
    PaymentOrchestrator ->> ProductHandler: StockDeductionRequestedEvent
    ProductHandler ->> ProductHandler: productService.deductStock()
    ProductHandler ->> PaymentOrchestrator: StockDeductedEvent
    Note over PaymentOrchestrator: Step 3: Coupon Usage (Optional)
    PaymentOrchestrator ->> Redis: markStepCompleted(DEDUCT_STOCK)
    alt 쿠폰 사용
        PaymentOrchestrator ->> CouponHandler: CouponUsageRequestedEvent
        CouponHandler ->> CouponHandler: couponService.useCoupon()
        CouponHandler ->> PaymentOrchestrator: CouponUsedEvent
        PaymentOrchestrator ->> Redis: markStepCompleted(USE_COUPON)
    end

    Note over PaymentOrchestrator: Step 4: Order Completion
    PaymentOrchestrator ->> OrderHandler: OrderCompletionRequestedEvent
    OrderHandler ->> OrderHandler: orderService.completeOrderForSaga()
    OrderHandler ->> PaymentOrchestrator: OrderCompletedEvent
    Note over PaymentOrchestrator: Saga 완료
    PaymentOrchestrator ->> Redis: markStepCompleted(COMPLETE_ORDER)
    PaymentOrchestrator ->> Redis: updateStatus(COMPLETED)
    PaymentOrchestrator ->> PaymentOrchestrator: PaymentSagaCompletedEvent
...
sequenceDiagram
    ...
    PaymentOrchestrator ->> ProductHandler: StockDeductionRequestedEvent
    ProductHandler ->> ProductHandler: productService.deductStock()
    Note right of ProductHandler: 재고 부족으로 실패
    ProductHandler ->> PaymentOrchestrator: StockDeductionFailedEvent
    Note over PaymentOrchestrator: 보상 트랜잭션 시작
    PaymentOrchestrator ->> Redis: updateStatus(COMPENSATING)
    PaymentOrchestrator ->> PaymentOrchestrator: PaymentSagaFailedEvent
    PaymentOrchestrator ->> PaymentOrchestrator: startCompensation()
    Note over PaymentOrchestrator: 완료된 단계들의 보상 실행
    PaymentOrchestrator ->> BalanceHandler: BalanceRefundRequestedEvent
    BalanceHandler ->> BalanceHandler: balanceService.charge() // 잔액 환불
    BalanceHandler ->> PaymentOrchestrator: BalanceRefundedEvent
    PaymentOrchestrator ->> OrderHandler: OrderCancellationRequestedEvent
    OrderHandler ->> OrderHandler: orderService.cancelOrderForSaga()
    OrderHandler ->> PaymentOrchestrator: OrderCancelledEvent
    PaymentOrchestrator ->> Redis: updateStatus(FAILED)
    PaymentOrchestrator -->> Client: BusinessException (실패)
```
...
1. **이벤트 기반 Orchestrator 패턴**: 중앙 집중식 제어로 복잡한 비즈니스 플로우 관리
2. **도메인별 핸들러 분리**: MSA 전환 시 서비스별 독립 배포 가능
3. **포괄적인 보상 트랜잭션**: 모든 단계에 대응하는 보상 로직 구현
4. **Redis 기반 상태 관리**: 빠른 상태 조회 및 TTL 기반 자동 정리
